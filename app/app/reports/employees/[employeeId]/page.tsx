...
import { LogisticsMarkerBar } from '@/components/LogisticsMarkers';
import { logisticsDisplayForStore, weekdayFromDateStr } from '@/lib/storeOrderReminders';
...
export default function EmployeeMonthlyReportPage() {
    ...
    try {
      const [empRes, assignRes, vacRes] = await Promise.all([
        ...
        supabase
          ...
          .select(
            '*, shift:shifts(*), store:stores(id,name,order_days,delivery_days,fresh_order_days,fresh_delivery_days)'
          )
  ...
  type TableRow =
    ...
    | { kind: 'work'; date: string; assignment: AssignmentRow; showDate: boolean };
  ...
  const tableRows = useMemo(() => {
    ...
    for (const d of calendarDays) {
      ...
      const dayRows = (assignmentsByDate.get(ds) ?? []).filter(
        (a) => (a.assignment_type ?? 'SHIFT') === 'SHIFT' && a.shift
      );
      if (dayRows.length === 0) {
        const other = assignmentsByDate.get(ds)?.[0];
        if (other) rows.push({ kind: 'work', date: ds, assignment: other, showDate: true });
        continue;
      }
      dayRows.forEach((a, idx) => {
        rows.push({ kind: 'work', date: ds, assignment: a, showDate: idx === 0 });
      });
    }
    return rows;
  }, [calendarDays, vacationOnDate, assignmentsByDate]);
                ...
                <tbody className="divide-y divide-gray-100">
                  {tableRows.length === 0 ? (
                    ...
                    tableRows.map((row) => {
                      ...
                      const logisticsDisplay = st
                        ? logisticsDisplayForStore(
                            st.id,
                            new Map([[st.id, st]]),
                            weekdayFromDateStr(row.date)
                          )
                        : null;
                              ...
                              <div className="flex flex-col gap-1">
                                ...
                                {logisticsDisplay ? (
                                  <LogisticsMarkerBar
                                    ...
                                    storeColor={resolveStoreColor(st?.color)}
