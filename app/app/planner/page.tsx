...
export default function PlannerPage() {
  ...
  const fetchAllData = useCallback(async (options?: { preserveView?: boolean }) => {
    ...
    try {
      ...
      const [employeesRes, storesRes, shiftsRes, assignmentsPaged, vacationsPaged] = await Promise.all([
        supabase
          .from('employees')
          .select('*')
        ...
        supabase
          .from('stores')
          .select('id,name,color,order_days,delivery_days,fresh_order_days,fresh_delivery_days,companies(name)')
          .order('name'),
        supabase.from('shifts').select('*').order('start_time'),
        fetchAllPaged<PlannerAssignmentRow>((from, to) =>
          supabase
        ...
        fetchAllPaged<Vacation>((from, to) =>
          supabase
            .from('vacations')
            .select('*')
      ...
      if (vacationsPaged.error) throw vacationsPaged.error;

      const assignmentsRes = { data: assignmentsPaged.data, error: null };
      const vacationsRes = { data: vacationsPaged.data, error: null };
          ...
          @media print {
            * {
            ...
            #planner-print-area table {
              width: 100% !important;
              font-size: clamp(11px, 2.35mm, 15px) !important;
              page-break-inside: avoid !important;
              break-inside: avoid !important;
            }

            #planner-print-area .planner-grid-print-layout table thead th {
              padding: 0.28rem 0.35rem !important;
            }

            #planner-print-area table .planner-print-day-cell {
              min-width: 15mm !important;
              padding: 0.2rem 0.25rem !important;
              vertical-align: middle !important;
            }

            #planner-print-area .planner-multi-shift-row {
              flex-wrap: nowrap !important;
            }

            #planner-print-area .planner-multi-shift-row > .planner-shift-block {
              min-width: 22mm !important;
              flex: 1 1 0 !important;
            }

            /* Larger numerics and shift times for PDF readability. */
            #planner-print-area table .planner-cell-hours {
              font-size: clamp(19px, 3.85mm, 30px) !important;
              line-height: 1.1 !important;
            ...
            #planner-print-area table .planner-pos-input-num {
              font-size: clamp(17px, 3.45mm, 26px) !important;
              line-height: 1.1 !important;
              font-weight: 600 !important;
            }
            ...
            #planner-print-area table .planner-header-daynum {
              font-size: clamp(22px, 4.4mm, 34px) !important;
            ...
            #planner-print-area table thead th.planner-print-employee-header-cell {
              ...
              font-size: clamp(12px, 2.4mm, 16px) !important;
            ...
            #planner-print-area table tbody th.planner-print-employee-name-cell span {
              font-size: clamp(24px, 4.35mm, 32px) !important;
            ...
            .planner-print-title {
              font-size: clamp(13px, 2.8mm, 18px) !important;
              margin: 0 0 6px !important;
            }

            .planner-print-company {
              font-size: clamp(14px, 2.9mm, 18px) !important;
              font-weight: 700 !important;
              text-align: center !important;
            ...
            #planner-print-area table thead tr:first-child th {
              font-size: clamp(11px, 2.2mm, 14px) !important;
            }
