...
async function uploadZipBackup(
  admin: SupabaseClient,
  filename: string,
  ...
  if (error) {
    throw new Error(
      `Failed to upload backup: ${error.message}`
    );
  }
...
export async function GET(req: NextRequest) {
  try {
    ...
    const backupSecret =
      process.env.BACKUP_API_SECRET || '';

    const providedSecret = getRequestSecret(req);

    const hasValidSecret =
      !!backupSecret &&
      providedSecret === backupSecret;

    // 🔥 AUTH LOGIC (FIXED)
    if (!isDev) {
      const allowed = isCron || hasValidSecret;
    ...
    const shouldUpload =
      isCron ||
      req.nextUrl.searchParams.get('store') === '1';

    if (shouldUpload) {
      const bucket = await uploadZipBackup(
        admin,
        zipFilename,
        zipBuffer
      );
      ...
      return NextResponse.json({
        ...
        bucket,
        file: zipFilename,
        tables: tableData.map(({ table, rows }) => ({
          table,
          count: rows.length,
        })),
      });
