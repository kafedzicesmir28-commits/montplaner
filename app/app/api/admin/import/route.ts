...
function buildAssignmentIdKey(row: ImportJsonRow) {
  const id = typeof row.id === 'string' ? row.id.trim() : '';
  return id || null;
}

async function filterExistingShiftAssignments(
  adminSelect: () => Promise<Array<{ id?: string | null }>>,
  rows: ImportJsonRow[]
) {
  if (!rows.length) return { rows, skipped: 0 };
  const existingRows = await adminSelect();
  ...
  const filtered = rows.filter((row) => {
    const composite = buildAssignmentIdKey(row);
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const tableName of BACKUP_TABLES) {
      ...
      if (skipDuplicates && rows.length > 0 && tableName === 'shift_assignments') {
        const result = await filterExistingShiftAssignments(
          async () => {
            const { data, error } = await admin.from('shift_assignments').select('id');
