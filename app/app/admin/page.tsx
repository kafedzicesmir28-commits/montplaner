...
export default function AdminPage() {
  ...
  const [loginLogs, setLoginLogs] = useState<OwnerLoginLogRow[]>([]);
  ...
  const loadLoginLogs = useCallback(async (accessToken: string) => {
    setLoadingLoginLogs(true);
    setLogsError('');
    try {
      const res = await fetch('/api/admin/logs?type=login&limit=100', {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error ?? 'Failed to load login logs');
      setLoginLogs((data.logs ?? []) as OwnerLoginLogRow[]);
      setLoginLogsOpen(true);
    } catch (e: unknown) {
      setLogsError(e instanceof Error ? e.message : 'Failed to load login logs');
    } finally {
      setLoadingLoginLogs(false);
    }
  }, []);

  const loadAuditLogs = useCallback(async (accessToken: string) => {
    setLoadingAuditLogs(true);
    setLogsError('');
    try {
      const res = await fetch('/api/admin/logs?type=audit&limit=100', {
        headers: { Authorization: `Bearer ${accessToken}` },
      });
      const data = await res.json();
      ...
      if (!res.ok) {
        const apiError =
          typeof data === 'object' && data !== null && 'error' in data
            ? String((data as { error?: string }).error ?? '')
            : '';
        throw new Error(apiError || text || 'Failed to load tickets');
      }
      const payload =
        typeof data === 'object' && data !== null ? (data as { tickets?: AdminTicket[] }) : {};
    ...
    const boot = async () => {
      ...
      try {
        ...
        try {
          ...
          setTickets([]);
          setTicketsError(
            ticketErr instanceof Error ? ticketErr.message : 'Failed to load support tickets'
          );
        }
      } catch (e: unknown) {
          ...
          {!loading && !error && overview ? (
            <>
              <section className="grid gap-3 md:grid-cols-3">
                ...
                <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                  <p className="text-sm text-gray-500">Latest owner login</p>
                  <p className="mt-2 text-sm font-medium">
                    {formatDateTime(overview.stats.latest_owner_login?.login_time ?? null)}
                  </p>
                  {overview.stats.latest_owner_login?.email ? (
              ...
              <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {overview.stats.login_logs_total} entries in database
                      {loginLogsOpen ? ` · showing latest ${loginLogs.length}` : ''}
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {loginLogsOpen ? (
                  <div className="mt-3 max-h-80 overflow-auto rounded-md border border-gray-100">
                    <table className="min-w-full divide-y divide-gray-200 text-sm">
              ...
              <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">
                      {overview.stats.audit_logs_total} total · {overview.stats.audit_events_24h} in last 24h
                      {auditLogsOpen ? ` · showing latest ${auditLogs.length}` : ''}
                    </p>
                  </div>
                  <button
                    type="button"
