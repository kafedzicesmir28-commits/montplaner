...
import { LogisticsMarkerCorner } from '@/components/LogisticsMarkers';
...
type StoreForPlanner = Pick<
  Store,
  'id' | 'name' | 'order_days' | 'delivery_days' | 'fresh_order_days' | 'fresh_delivery_days'
> & { color?: string | null };
...
export type PlannerCellProps = {
  ...
  /** Add another shift block for this day (opens assign modal). */
  onClickAssignAdd?: () => void;
  onActivateAssignment?: (assignmentId: string) => void;
  editingAssignmentId?: string;
  allShiftAssignments?: PlannerCellProps['dayAssignments'];
...
export default function PlannerCell({
  ...
  const weekday = useMemo(() => weekdayFromDateStr(dateStr), [dateStr]);
  const storesById = useMemo(() => {
    const map = new Map<string, Store>();
    for (const s of stores) {
      map.set(s.id, s as Store);
    }
    return map;
  }, [stores]);
  ...
  const persist = useCallback(async (closeAfterSave = false) => {
    ...
    const selectedShift = shifts.find((s) => s.id === shiftId);
    const overlapStart = normalizedStart || (selectedShift ? hhmmFromShiftTime(selectedShift.start_time) : '');
    const overlapEnd = normalizedEnd || (selectedShift ? hhmmFromShiftTime(selectedShift.end_time) : '');
    if (
      overlapStart &&
      overlapEnd &&
      hasShiftTimeOverlap(overlapStart, overlapEnd, dayAssignments, shifts, assignment?.id)
    ) {
      setSaveError(t.plannerShiftOverlap);
      return;
    }
  ...
  const applyShiftQuick = useCallback(async (nextShiftId: string) => {
    ...
    const nextStart = hhmmFromShiftTime(selectedShift.start_time);
    const nextEnd = hhmmFromShiftTime(selectedShift.end_time);
    if (
      nextStart &&
      nextEnd &&
      hasShiftTimeOverlap(nextStart, nextEnd, dayAssignments, shifts, assignment?.id)
    ) {
      setSaveError(t.plannerShiftOverlap);
      setSaving(false);
      return;
    ...
    try {
      const breakMins = snapToPlannerBreakMinutes(selectedShift.break_minutes ?? 0);
      const result = assignment?.id
        ? await upsertQuickPlannerShift({
            employeeId,
            dateStr,
            shiftId: nextShiftId,
  ...
  const handleCellClick = () => {
    ...
    const openClickAssignModal =
      Boolean(onClickAssignEmpty) && !pendingStoreId && !statusOnly && shiftBlockCount === 0;
  ...
  const shiftBlocks = sortedShiftAssignments(allShiftAssignments ?? dayAssignments, shifts);
  const shiftBlockCount = shiftBlocks.length;
  const cellLogisticsOverlay = useMemo(
    () =>
      logisticsOverlayForCell(storesById, weekday, {
        forceStoreId,
        pendingStoreId: pendingStoreId ?? undefined,
      }),
    [storesById, weekday, forceStoreId, pendingStoreId]
  );
  const pendingLogisticsOverlay =
    shiftBlockCount === 0 && cellLogisticsOverlay != null ? cellLogisticsOverlay : null;

  const editorMinWidth = isEditing ? 220 : undefined;
  /** Fits "22:00 – 06:00" + shift code without ellipsis. */
  const perShiftBlockWidth = printLayout ? 116 : 100;
  const multiShiftGapPx = 4;
  const cellPadPx = 8;
  const baseCellMinWidth = printLayout ? 124 : 96;
  const multiShiftLayout = shiftBlockCount > 1;
  const cellMinWidth =
    editorMinWidth ??
    (multiShiftLayout
      ? cellPadPx +
        shiftBlockCount * perShiftBlockWidth +
        (shiftBlockCount - 1) * multiShiftGapPx
      : baseCellMinWidth);
  const cellMaxWidth = isEditing ? 320 : multiShiftLayout ? undefined : printLayout ? 168 : 132;
  const cellWidth = multiShiftLayout && !isEditing ? cellMinWidth : undefined;
  const cellMinHeight = printLayout ? 92 : 72;
  const perBlockMinWidthClass = printLayout ? 'min-w-[116px]' : 'min-w-[100px]';
  const blockMinHeightClass = printLayout ? 'min-h-[54px]' : 'min-h-[44px]';
  const statusBlockHeightClass = printLayout ? 'h-[72px]' : 'h-[58px]';
  const shiftHoursClass = printLayout ? 'text-[16px]' : 'text-[14px]';
  const shiftCodeClass = printLayout ? 'text-[12px]' : 'text-[10px]';
  const shiftBreakClass = printLayout ? 'text-[13px]' : 'text-[11px]';
  const cardStyle = {
    backgroundColor,
    color,
  ...
  return (
    <td
      ...
      onDrop={handleDrop}
      className={`align-top p-0 text-left${printLayout ? ' planner-print-day-cell' : ''} ${
      ...
      style={{
        ...
        width: cellWidth,
        maxWidth: printLayout && isEditing ? 340 : cellMaxWidth,
    ...
    >
      {!isEditing && isBirthday ? (
        <div className="pointer-events-none absolute left-1 top-1 z-10">
          <div
            className="rounded-full border border-amber-300 bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold text-amber-900 shadow-sm"
            title="Birthday"
            aria-label="Birthday"
          >
            B-Day
      ...
      {isEditing && assignment && isAssignmentStatusOnly && !isVacation ? (
        <div className="px-0.5 py-0.5" onClick={stop} onMouseDown={stop}>
          <div
            className={`mb-1 flex ${blockMinHeightClass} items-center justify-center rounded-md px-1 py-2 text-center font-semibold leading-tight ${
              printLayout ? 'text-[13px]' : 'text-[11px]'
            }`}
            style={cardStyle}
          >
        ...
        >
          <div
            className={`flex ${statusBlockHeightClass} items-center justify-center rounded-md text-center font-semibold leading-tight ${
              printLayout ? 'text-[13px]' : 'text-[11px]'
            }`}
            style={cardStyle}
          >
        ...
        <div className="p-1">
          <div
            className={
              multiShiftLayout
                ? 'planner-multi-shift-row flex flex-row flex-nowrap items-stretch justify-start gap-1'
                : 'flex flex-col'
            }
          >
            {shiftBlocks.map((row, blockIndex) => {
              const blockShift = shifts.find((s) => s.id === row.shift_id);
              if (!blockShift) return null;
              const blockRange = workingRange(row as PlannerAssignment, blockShift);
              const blockBreak = effectiveBreakMinutes(row as PlannerAssignment, blockShift);
              const blockCode = blockShift.code?.trim() || blockShift.name?.trim() || '';
              const blockStoreId = row.store_id ?? forceStoreId ?? selectedStoreId;
              const blockBg = blockStoreId ? getStoreColor(blockStoreId, stores) : backgroundColor;
              const blockFg = contrastingForeground(blockBg);
              const isPrimaryBlock = blockIndex === 0;
              const blockLogistics = logisticsDisplayForStore(blockStoreId, storesById, weekday);
              return (
                <div
                  key={row.id}
                  className={`planner-shift-block group/block relative ${
                    multiShiftLayout ? `shrink-0 flex-none ${perBlockMinWidthClass}` : 'min-w-0 w-full'
                  }`}
                >
                  <button
                    type="button"
                    ...
                    onClick={(e) => {
                      ...
                      onActivateAssignment?.(row.id);
                    }}
                    className={`relative flex h-full w-full ${blockMinHeightClass} flex-col items-center justify-center gap-0 rounded-md px-1 py-0.5 text-center font-medium leading-tight transition hover:brightness-[0.97] disabled:cursor-default ${shiftCodeClass} ${
                      isPrimaryBlock ? '' : 'border border-dashed border-white/70'
                    }`}
                    style={{ backgroundColor: blockBg, color: blockFg }}
                  >
                    <LogisticsMarkerCorner display={blockLogistics} />
                    <div
                      className={`font-semibold tracking-tight ${multiShiftLayout ? 'whitespace-nowrap' : 'max-w-full truncate'}`}
                      title={blockCode}
                    >
                      {blockCode || '—'}
                    </div>
                    <div
                      className={`planner-cell-hours mt-0.5 font-medium tabular-nums leading-tight ${shiftHoursClass} ${
                        multiShiftLayout ? 'whitespace-nowrap' : 'min-w-0 max-w-full truncate'
                      }`}
                      title={blockRange || undefined}
                    >
                      {blockRange || '—'}
                    </div>
                    {blockBreak > 0 ? (
                      <div
                        className={`planner-cell-numeric mt-0.5 tabular-nums leading-tight opacity-70 ${shiftBreakClass} ${
                          multiShiftLayout ? 'whitespace-nowrap' : 'min-w-0 max-w-full truncate'
                        }`}
                      >
                        {t.plannerPauseAbbrev} {blockBreak}
                      </div>
                    ) : null}
                  </button>
                  {!readOnly && !isUnavailable ? (
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        void (async () => {
                          ...
                          try {
                            ...
                            await onSaved();
                            notifyPlannerAssignmentsChanged();
                            if (assignment?.id === row.id) onCloseCellEdit?.();
                          } catch (err: unknown) {
                      ...
                      title={t.deleteAssignment}
                      aria-label={t.deleteAssignment}
                      className="absolute right-0.5 top-0.5 z-10 flex h-4 w-4 items-center justify-center rounded bg-white/90 text-[11px] font-bold leading-none text-red-700 shadow-sm ring-1 ring-red-200 opacity-0 transition-opacity hover:bg-red-50 group-hover/block:opacity-100"
                    >
                      ×
                    </button>
                  ) : null}
          ...
          {onClickAssignAdd && !readOnly && !isUnavailable ? (
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              className="mt-1 w-full rounded border border-gray-300 bg-white/80 py-0.5 text-[10px] font-semibold text-gray-800 hover:bg-white"
            >
              {t.plannerAddShift}
            </button>
          ) : null}
        ...
        <div className="p-1">
          <div
            className={`flex ${statusBlockHeightClass} flex-col items-center justify-center rounded-md px-1 text-center font-semibold leading-snug ${
              printLayout ? 'text-[13px]' : 'text-[11px]'
            }`}
        ...
        <div className="p-1">
          <div
            className={`relative flex ${statusBlockHeightClass} flex-col items-center justify-center rounded-md px-1 text-center font-medium leading-snug ${
              printLayout ? 'text-[12px]' : 'text-[11px]'
            }`}
            style={cardStyle}
          >
            <LogisticsMarkerCorner display={pendingLogisticsOverlay} />
            ...
            <div
              className={`planner-cell-hours mt-0.5 min-w-0 max-w-full truncate font-medium tabular-nums leading-tight ${
                printLayout ? 'text-[16px]' : 'text-[15px]'
              }`}
            >
              —
            </div>
        ...
        <div
          className={`flex h-full items-center justify-center px-1 py-2 text-center font-normal opacity-70 ${
            printLayout ? 'text-[12px]' : 'text-[10px]'
          }`}
        >
          —
        </div>
