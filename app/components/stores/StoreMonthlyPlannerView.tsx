  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const [employeesRes, storesRes, shiftsRes, assignmentsRes, allAssignmentsRes, vacationsRes] = await Promise.all([
        ...
        supabase
          .from('stores')
          .select('id,name,color,order_days,delivery_days,fresh_order_days,fresh_delivery_days')
          .order('name'),
      ...
      const storeIdsByEmployeeDate = new Map<string, Set<string>>();
      for (const row of allRows) {
        const key = `${row.employee_id}:${row.date}`;
        if (!row.store_id) continue;
        let set = storeIdsByEmployeeDate.get(key);
        if (!set) {
          set = new Set();
          storeIdsByEmployeeDate.set(key, set);
        }
        set.add(row.store_id);
      }
      const unavailableKeys: string[] = [];
      for (const [key, storeIds] of storeIdsByEmployeeDate) {
        if (!storeIds.has(storeId)) unavailableKeys.push(key);
      }
