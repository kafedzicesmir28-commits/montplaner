...
export type PlannerClickAssignModalProps = {
  ...
  dayAssignments: Array<
    Pick<ShiftAssignment, 'id' | 'shift_id' | 'assignment_type' | 'custom_start_time' | 'custom_end_time'>
  >;
  stores: StoreRow[];
  shifts: Shift[];
  onClose: () => void;
...
export function PlannerClickAssignModal({
  ...
  const handlePickShift = useCallback(
    async (shift: Shift) => {
      ...
      setSaving(true);
      setError(null);
      const nextStart = formatClock(shift.start_time);
      const nextEnd = formatClock(shift.end_time);
      if (
        hasShiftTimeOverlap(
          nextStart,
          nextEnd,
        ...
        setSaving(false);
        setError(t.plannerShiftOverlap);
        return;
      }
      const breakMinutes = snapToPlannerBreakMinutes(shift.break_minutes ?? 0);
    ...
    [assignmentId, availableShifts, dateStr, dayAssignments, employeeId, onSaved, resetAndClose, shifts, storeId]
  );
    ...
    >
      ...
      >
        <h2 id="planner-click-assign-title" className="text-lg font-semibold text-gray-900">
          {assignmentId ? t.plannerClickAssignTitle : t.plannerAddShift}
        </h2>
