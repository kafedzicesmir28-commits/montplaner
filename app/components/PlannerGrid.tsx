...
import { pickCellDisplayAssignments } from '@/lib/plannerDayAssignments';
...
export type PlannerGridProps = {
  ...
  /** Larger cells and typography for browser print / Save as PDF. */
  printLayout?: boolean;
  forceStoreId?: string;
...
function calculateWeeklyHours(
  ...
  return weekDays.reduce((total, day) => {
    ...
    const dayAssignments = assignmentByKey.get(`${employeeId}:${dateStr}`) ?? [];
    const dayTotal = dayAssignments.reduce((sum, assignment) => {
      const shift = assignment?.shift_id ? shiftById(shifts, assignment.shift_id) : undefined;
      return sum + getHoursForAssignment(assignment, shift);
    }, 0);
...
export default function PlannerGrid({
  ...
  const [clickAssignTarget, setClickAssignTarget] = useState<{
    ...
    dayAssignments: AssignmentWithStore[];
  } | null>(null);
  ...
  return (
    <div
      ref={gridRef}
      className={`min-w-0 rounded-lg border border-gray-200 bg-white shadow-sm${printLayout ? ' planner-grid-print-layout' : ''}`}
    >
            ...
            <thead>
              <tr className="bg-[#f8fafc]">
                <th
                  ...
                  style={{
                    minWidth: printLayout ? 52 : 44,
                    width: printLayout ? 52 : 44,
                  }}
                >
                  #
                </th>
                <th
                  scope="col"
                  rowSpan={2}
                  className={`sticky top-0 z-30 border border-gray-200 border-r border-gray-200 bg-white px-2 py-1.5 align-middle text-left font-bold uppercase tracking-wide text-gray-700 planner-print-employee-header-cell ${
                    printLayout ? 'text-[13px]' : 'text-[11px]'
                  }`}
                  style={{ left: printLayout ? 52 : 44, minWidth: printLayout ? 148 : 132 }}
                >
                  {t.employee}
                </th>
                ...
                {weekSegments.map((seg, segIdx) => (
                  <Fragment key={`hdr-${seg.weekYear}-W${seg.weekNumber}-${formatDate(seg.days[0]!)}`}>
                    {seg.days.map((day, dayIndex) => (
                      ...
                      >
                        <div
                          className={`planner-header-daynum font-bold tabular-nums leading-tight text-gray-800 ${
                            printLayout ? 'text-[20px]' : 'text-[15px]'
                          }`}
                        >
                          {day.getDate()}
                        </div>
                        <div
                          className={`font-semibold text-gray-500 ${printLayout ? 'text-[12px]' : ''}`}
                        >
                ...
                return (
                  ...
                  >
                    <input
                      ...
                      className={`planner-pos-input-num w-full rounded border border-gray-300 bg-white px-1 py-0.5 text-center font-semibold tabular-nums text-gray-800 ${
                        printLayout ? 'text-base' : 'text-sm'
                      }`}
                    />
                  </th>
                  <th
                  ...
                  {(() => {
                    ...
                    const chunk = seg.days.map((day, dayIndex) => {
                      const dateStr = formatDate(day);
                      const dayAssignments = assignmentByKey.get(`${employee.id}:${dateStr}`) ?? [];
                      const cellKey = `${employee.id}:${dateStr}`;
                      const { statusAssignment, shiftAssignments, primaryShift } =
                        pickCellDisplayAssignments(dayAssignments, shifts);
                      const displayAssignment = statusAssignment ?? primaryShift;
                      const editingThisCell =
                        editingCell?.employeeId === employee.id &&
                        editingCell?.dateStr === dateStr;
                      const editingAssignmentId = editingThisCell
                        ? editingCell?.assignmentId
                        : undefined;
                      const assignment =
                        editingThisCell && editingAssignmentId
                          ? dayAssignments.find((a) => a.id === editingAssignmentId) ??
                            displayAssignment
                          : displayAssignment;
                      ...
                      const shift = assignment?.shift_id ? shiftById(shifts, assignment.shift_id) : undefined;
                      const cellHours = dayAssignments.reduce((sum, row) => {
                        const rowShift = row.shift_id ? shiftById(shifts, row.shift_id) : undefined;
                        return sum + getHoursForAssignment(row, rowShift);
                      }, 0);
                      weekTotal += cellHours;
                      ...
                      return (
                        <PlannerCell
                          ...
                          isEditing={!readOnly && !isVacation && !isUnavailable && editingThisCell}
                          editingAssignmentId={editingAssignmentId}
                          allShiftAssignments={shiftAssignments}
                          readOnly={readOnly}
                          ...
                          onActivate={() => {
                            if (!readOnly && !isVacation && !isUnavailable) {
                              setEditingCell({
                                employeeId: employee.id,
                                dateStr,
                          ...
                          onActivateAssignment={(assignmentId) => {
                            if (!readOnly && !isVacation && !isUnavailable) {
                              setEditingCell({ employeeId: employee.id, dateStr, assignmentId });
                            }
                          }}
                          onClickAssignAdd={
                            !readOnly && stores.length > 0 && !statusAssignment
                              ? () =>
                          ...
                          onClickAssignEmpty={
                            !readOnly && stores.length > 0 && shiftAssignments.length === 0
                              ? () =>
                                  setClickAssignTarget({
                                    employeeId: employee.id,
                                    dateStr,
                          ...
                          onCloseCellEdit={() => setEditingCell(null)}
                    ...
                    if (printWeeklyTotals && si < weekSegments.length - 1) {
                      chunk.push(
                        <td
                          ...
                          className={`border border-gray-200 bg-gray-100 px-2 py-1 text-right font-semibold tabular-nums text-gray-800 planner-cell-hours ${
                            printLayout ? 'text-sm' : 'text-[11px]'
                          }`}
                    ...
                    if (printWeeklyTotals && si === weekSegments.length - 1) {
                      chunk.push(
                        <td
                          ...
                          className={`border border-gray-200 bg-blue-50 px-2 py-1 text-right font-bold tabular-nums text-blue-900 planner-cell-hours ${
                            printLayout ? 'text-sm' : 'text-[11px]'
                          }`}
      ...
      {clickAssignTarget ? (
        <PlannerClickAssignModal
          ...
          dayAssignments={clickAssignTarget.dayAssignments}
          stores={stores}
          shifts={shifts}
