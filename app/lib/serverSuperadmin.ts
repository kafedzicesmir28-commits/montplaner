...
function getEnv(name: string) {
  ...
  if (!value) {
    if (name === 'SUPABASE_SERVICE_ROLE_KEY') {
      throw new Error(
        'Missing SUPABASE_SERVICE_ROLE_KEY. Add it to app/.env.local from Supabase → Project Settings → API → service_role (secret). Restart npm run dev. Keep this key server-side only.'
      );
    }
